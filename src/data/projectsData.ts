export const projectData = [
  {
    title: "London Taste Models",
    img: import.meta.env.VITE_BASE_URL + "/images/taste-model.png",
    description: "Online model booking and escort services app. React frontend laravel larvel backend with admin dashboard for managing the website content",
    link: "https://londontastemodels.co.uk",
  },
  {
    title: "Laundramoon  (Under development)",
    img: import.meta.env.VITE_BASE_URL + "/images/laundramoon.png",
    description:
      "This is an online laundry service app where user can book laundry services.",
    link: "https://laundramoon.infoster.in/",
  },
  {
    title: "Jivansurksha - Figma template to html",
    img: import.meta.env.VITE_BASE_URL + "/images/jivansurksha.png",
    description: "A landing page created for jivansurksha website",
    link: "https://salman3001.github.io/jivansurksha-figma-project-2/",
  },
  {
    title: "BuyZone (Demo E-commerce App)",
    img: import.meta.env.VITE_BASE_URL + "/images/buyzone.png",
    description:
      "This is a full stack Ecommerce application. Built with React , Nestjs(Nodejs), Mongodb. It has many features like cart, Reviews, authentication, Roles, products sorting and filtering , products search, product reviews and more.",
    link: "https://salman3001.github.io/buyzone-ecommerce-app-mern/",
  },
  {
    title: "Multi App Server -Nest Js",
    img: import.meta.env.VITE_BASE_URL + "/images/nestjs.png",
    description:
      "Nest js backend for Rest API's. Deployed on Render.com. Explore code on github",
    link: "https://github.com/salman3001/nestjs-server/",
  },
  {
    title: "React Calculator",
    img: import.meta.env.VITE_BASE_URL + "/images/calculator.jpg",
    description:
      "A fully functional caluclator. Made with react. Tested with jest library. It has all common features of a calculator. It can be used with touch, mouse or keyboard ",
    link: "https://salman3001.github.io/calculator-react/",
  },
  {
    title: "Figma Project",
    img: import.meta.env.VITE_BASE_URL + "/images/figma-logo.png",
    description:
      "A figma template was converted to HTML pages using tailwind CSS on clients request. Explore the developed pages.",
    link: "https://salman3001.github.io/figma-project-1/",
  },
  {
    title: "Buyzone Express server",
    img: import.meta.env.VITE_BASE_URL + "/images/express.png",
    description:
      "Express js backend. Explore the code on github",
    link: "https://github.com/salman3001/buyzone-express-server/",
  },
  {
    title: "NPM Pakage - React State Manager",
    img: import.meta.env.VITE_BASE_URL + "/images/npm-logo.png",
    description:
      "A simple react state manager developed to sipmlify the use of react useContext and useReducer hooks. Written with tyepscript, deployed as NPM package.",
    link: "https://github.com/salman3001/react-state-manager/",
  },
  {
    title: "Portfolio Project-By Meta (Facebook)",
    img: import.meta.env.VITE_BASE_URL + "/images/Meta-Logo.png",
    description:
      "This project was developed during my front development course by META as a final assignment project. Includes state management, form validations etc",
    link: "https://salman3001.github.io/meta-final-project/",
  },
];
